...
> **核心数据**: 3个月完成 9 个业务模块 + 1个上线交付项目 | 已上线项目（智慧药房配送系统，2000行代码） | 8个模块独立技术实现 + 1个模块从0到1自主设计  
...
| **产品设计** | 1个上线项目（智慧药房，已服务真实患者）+ 1个自主设计（DRG）+ 8个重构优化 | [1700+行PRD文档](./product-output/prd/) |
...
- ✅ **闭环能力**: 从业务分析→产品设计→技术实现→上线交付，智慧药房项目已服务真实患者
...
|------|------|------|
| **开发周期** | 3个月 | 传统方式预计需6个月+ |
| **业务模块** | 9个完整模块 | 8个重构模块（独立技术实现）+ 1个DRG模块（从0到1自主设计） |
...
```
业务洞察 → 产品设计 → 技术实现 → 上线交付 → 治理框架
    ↓         ↓          ↓          ↓          ↓
1.5万行   1700+行    2万+行代码  智慧药房   V.A.R.C.
业务文档   PRD文档    重构+DRG   (已上线)   工作流模板
```

**核心里程碑**：
- ✅ 智慧药房配送系统：**已上线交付**（2000行代码，复杂业务规则）
- ✅ 8个模块生产优化：代码精简62%，类型覆盖100%
- ✅ DRG模块自主设计：从0到1完整闭环
- ✅ 方法论沉淀：RAC-FLOW认知增强协议
...
业务需求: 实习期间业务方提出场景和痛点
   ↓
人工: 理解需求，设计分层架构（types→store→api→composables→components）
   ↓
AI:   根据架构生成基础代码（大量辅助）
   ↓
人工: Review业务逻辑、处理边界情况、优化类型定义、自测交付
   ↓
结果: 3周完成传统需6周的工作量，独立负责前端技术全流程
```

**关键**: 需求来源业务方，技术方案我自主设计，代码我独立编写（AI辅助），质量我人工把控。

#### 场景2：DRG模块从0到1自主设计

```
人工: 从拔牙经历洞察DRG改革对医院的影响（自主发现需求）
   ↓
人工: 设计产品功能（DRG费用分析、飞检风险评估等）
   ↓
人工: 设计技术架构（复杂业务逻辑、状态流转）
   ↓
AI:   辅助生成基础代码
   ↓
人工: 实现核心业务逻辑（36个文件，10k+行），自测交付
   ↓
结果: 唯一从需求洞察到产品落地完整自主设计的模块
```

**关键**: 这是我唯一一个从0到1全流程自主的模块，证明了我完整的产品设计能力。
...
> **📎 原型文件说明**：PRD文档及高保真原型文件已移出公开仓库，面试时可线下展示。
...
| 文档 | 定位 | 规模 | 内容亮点 |
|------|------|------|----------|
| 产品设计版PRD | 面向设计/开发 | 816行 | 设计规范+交互规范+8个核心页面详细设计 |
| 功能规格版PRD | 面向实施 | 901行 | 43个模块+200+页面路径+组件库说明 |
...
### 🌟 核心项目：XX医院智慧药房配送系统（示例项目）

**项目背景**：参与医疗信息化项目实践，服务于XX医院，解决患者药品配送的复杂业务场景。

**业务复杂度**：
- **多入口场景**：标准流程入口 + 扫码直达入口
- **多药品类型**：自煎中药 + 代煎中药，业务规则完全不同
- **多配送方式**：窗口自取、快递配送、仅填地址（无需配送）
- **用户迁移**：老小程序用户历史数据无缝迁移

**技术方案**：
```
核心难点：业务规则组合爆炸（2种入口 × 2种药品类型 × 3种配送方式）

解决方案：策略路由引擎（应对需求频繁变更）
├── 演进过程：MVP版本用if-else（4种规则）→ 第二次迭代增加患者类型维度（8种规则）→ 改用策略模式避免维护困难
├── 核心设计：deliveryType + tcmDecoctionIndicator 组合匹配策略
├── 实际收益：第三次迭代新增冷链配送选项，仅新增2个策略文件（20行代码），半天完成开发和测试
└── 输出：当前场景下合法的配送方式选项

技术亮点：
- 策略模式：应对需求频繁变更（第三次迭代半天完成 vs if-else预计2天）
- OpenID同步：3次重试+降级处理，100%成功率，零感知切换
- 质量保障：Sentry监控 + 医院反馈群，3个小问题及时发现并修复
- 代码规模：3个页面，近2000行代码，100% TypeScript类型覆盖
```

**项目成果**：
- ✅ 项目实践，完成药品配送全流程功能设计与开发
- ✅ 严重故障（影响取药）：0起；线上小问题：3起（均已修复），有完整监控和响应流程
- ✅ 老用户迁移成功率100%，OpenID同步零感知切换
- ✅ 窗口排队取药患者减少40%（医院信息科反馈）

---

### 其他项目经历

| 项目类型 | 数量/规模 | 分工说明 | 核心成果 |
|----------|----------|---------|---------|
| **生产优化** | 8个模块 | 参与优化已上线的患者端系统，主导前端重构 | 代码精简62%，类型覆盖100% |
| **自主设计** | DRG结算模块 | 从0到1自主设计：业务洞察→产品设计→技术实现 | 36个文件，10k+行，9种设计模式 |
...
**需求分析阶段**：
- **用户调研**：访谈医院药房窗口工作人员3次，发现高峰期患者排队等待超30分钟、选错配送方式导致白跑等痛点
- **痛点识别**：自煎患者误选快递（白白支付运费）、代煎患者不知道需等2小时
- **优先级决策**：和leader、医院信息科共同确定MVP（核心配送选择）vs 二期（配送进度追踪）

**产品设计阶段**：
- **流程优化**：3个页面减少用户决策步骤，智能默认选项（有历史地址默认快递，无地址默认窗口）
- **异常处理**：弱网环境、地址超长、库存不足等10+异常场景的处理方案
- **数据验证**：上线后窗口排队患者减少40%，"选错配送方式"投诉降为0

**与技术开发的区别**：不仅是"把功能做出来"，而是"解决患者取药难痛点，用数据验证方案有效性"
...
**已上线项目（智慧药房配送系统）**：
- **项目分工**：与leader合作完成
  - leader负责：需求对接（与医院信息科沟通）、后端接口设计、项目进度把控、上线部署
  - 我负责：前端技术方案设计（策略路由引擎）、3个页面开发（近2000行）、老用户迁移方案实现、自测
- **技术主导点**：策略模式设计（应对需求频繁变更）、OpenID同步方案设计、复杂业务规则梳理
- **协作模式**：leader传达需求→我设计技术方案→评审确定→我独立开发→联调测试→leader部署上线
- **状态**：已上线交付，服务真实患者，我能独立解释每行核心代码逻辑

**8个重构模块（住院管家、治疗预约、问卷等）**：
- 需求来源：实习期间业务方提出的场景和痛点
- 我的职责：理解需求后，独立负责前端技术全流程（架构设计→代码实现→自测）
- 技术方案：我自主设计，参考Vue3社区最佳实践（正常学习，非闭门造车）
- 代码编写：我独立编写，AI辅助生成部分样板代码

**1个DRG模块（医保结算）**：
- 需求来源：我自己从DRG政策研究中洞察的业务需求
- 我的职责：从需求洞察→产品设计→技术实现，全流程自主设计
- 特殊意义：这是唯一一个从0到1完全由我主导的模块

**诚实边界**：
- 智慧药房项目是与leader合作，我主导前端技术实现
- 8个重构模块的需求来自业务方，我负责技术实现
- DRG模块是我唯一全流程自主设计的
- 我使用AI工具提效（但我能解释每行核心代码）

我的价值在于：在真实业务约束下，完成了从上线交付到技术优化的完整能力闭环。
...
- **作者**: 叶泽宇
- **仓库**: https://github.com/code-and-debug/internship-showcase
